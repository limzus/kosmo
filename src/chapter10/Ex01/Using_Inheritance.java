...
// 부모 클래스(Super class) : 자식 클래스의 공통적인 특징
...
//자식 클래스 : 부모 클래스의 필드와 메소드를 그대로 상속 받는다.
	//	-- 자식 클래스 만의 특징이 적용된 필드와 메소드를 선언하
class Student extends Human { //학생 // Student class는 Human  클래스를 상속 받는다.
	
	//부모클래스의 필드와 메소드가 그대로 내려온다.
	//자식클래스의 특징만 필드을 가진 필드와 메소드를 선언.
		int studentID;		//자식 클래스에서 선언된 필드
		void goToschool() {	//자식 클래스에서 선언된 메소드
...
public class Using_Inheritance {
	...
	public static void main(String[] args) {
		...
		//3. worker클래스 객체 생성 (자식)
		System.out.println("== Worker 객체 출력==");
		Worker extends =  Worker1();
